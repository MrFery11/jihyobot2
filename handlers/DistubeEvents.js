        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setAuthor({
                name: `Agregada a la fila de reproducción`,
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setAuthor({
                name: `Playlist agregada a la fila`,
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setTitle(
                `No se encontró una canción: \`${queue?.songs[0].name}\``
              ),
          ],
        })
        .then((msg) => {
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`La fila ha terminado, no hay más canciones!`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
      ...
      message.channel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`No puedo buscar \`${quary}\``),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(
                `${client.config.emoji.ERROR} No se encontraron resultados para \`${quary}\`!`
              ),
          ],
        })
        .then((msg) => {
        ...
        queue.textChannel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(client.config.embed.color)
                .setDescription("El canal de voz está vacío, me desconectaré..."),
            ],
          })
          .then((msg) => {
            setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Necesitas unirte a un canal de voz!**`
                );
              } else if (
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Necesitas unirte a __mi__ canal de voz! **`
                );
              } else if (!queue) {
                send(
                  interaction,
                  `** ${client.config.emoji.ERROR} No estoy reproduciendo música hasta ahora **`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Autoplay activado !! **`
                );
              } else {
                queue.toggleAutoplay();
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Autoplay desactivado !!**.`
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `**${client.config.emoji.SUCCESS} se saltó a la siguiente canción**.`
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Música detenida !!**.`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Fila continuada!! **`
                );
              } else if (!queue.paused) {
                await queue.pause();
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Fila detenida !! **`
                );
              }
            }
            break;
          case "loop":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} Loop desactivado !! **`
                );
              }
            }
            break;
